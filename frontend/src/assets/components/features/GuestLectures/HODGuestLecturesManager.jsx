...
const HODGuestLecturesManager = ({ userRole, selectedDept }) => {
    ...
    const generateExcelReport = async () => {
        ...
        saveAs(new Blob([buffer]), "Department_Guest_Lectures_Report.xlsx");
    };
    ...
    const grantAccess = async (facId) => {
        try {
            await axios.post(`${import.meta.env.VITE_BACKEND_URL}/toggle-guest-lecture-permission`, {
                id: facId,
                allowed: true
            });
    ...
    const activeApprovers = deptFaculty.filter(f => f.permissions?.canManageGuestLectures);

    return (
        <div className="std-page-container workshops-container">
            <div className="std-page-header">
                <h2>Department Guest Lectures</h2>
            </div>
            ...
            {activeTab === 'overview' && (
                <>
                    <div className="achievements-toolbar">
                        <div className="toolbar-text">All Faculty Guest Lectures</div>
                    ...
                    {showAddForm && (
                        <div className="user-picker-box" style={{ border: '1px solid #cbd5e1', padding: '15px', borderRadius: '8px', backgroundColor: '#f8fafc' }}>
                            ...
                            <div className="user-results-list" style={{ maxHeight: '200px', overflowY: 'auto' }}>
                                {deptFaculty
                                    .filter(f => !f.permissions?.canManageGuestLectures && f.username.toLowerCase().includes(accessSearch.toLowerCase()))
                                    .map(fac => (
...
export default HODGuestLecturesManager;
